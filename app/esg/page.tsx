...
export default function ESGPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Always use WSDOT fleet data for this demo
        const data = WSDOT_FLEET.map((v) => ({
          id: v.mmsi,
          name: v.name,
          type: v.type,
          mmsi: v.mmsi,
        ...
        // Even on error, fall back to WSDOT fleet data
        const fallbackData = WSDOT_FLEET.map((v) => ({
          id: v.mmsi,
          name: v.name,
          type: v.type,
          mmsi: v.mmsi,
  ...
  const runImpactAnalysis = useCallback((scenarioType: string, parameters: Record<string, unknown>) => {
    ...
    setTimeout(() => {
      ...
      const vesselIds = WSDOT_FLEET.slice(0, 3).map(v => v.mmsi);
...
<head>
  ...
  <title>WSDOT Ferries ESG Report - ${reportDate}</title>
  <style>
...
<body>
  <div class="container">
    <div class="header">
      <h1>🌿 WSDOT Ferries ESG Report</h1>
    ...
    <div class="footer">
      <p>This report was automatically generated by WSDOT Ferry Fleet Intelligence Platform</p>
      <p>© ${new Date().getFullYear()} WSDOT Ferry Operations</p>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    },
                    {
                      ...
                      type: 'new_project',
                      icon: Target,
                      title: 'New Ferry Route',
                      description: 'Evaluate adding a new cross-Sound ferry service',
                      params: { estimatedCost: 15000000, requiredVesselTypes: ['ferry'] },
                      color: 'blue',
                    },
                    {
